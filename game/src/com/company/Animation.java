...
public class Animation {
	...
	public void drawAnimation(Graphics g, double x, double y, int ispl ,Game game, double velx, double vely){
		//g.drawImage(currentImg, (int)x - offset, (int)y, null);
		if (ispl==1){
			int vx = (int) -(game.mx-x);
			int vy = (int)(game.my-y);
			AffineTransform at = new AffineTransform();
			at.translate(x,y);
			at.rotate(vy,vx,16,11);
			Graphics2D g2d = (Graphics2D) g;
			g2d.drawImage(currentImg, at, null);
		}
		else if(ispl==2){
			AffineTransform at = new AffineTransform();
			at.translate(x,y);
			at.rotate(velx,vely,16,16);
			Graphics2D g2d = (Graphics2D) g;
			g2d.drawImage(currentImg, at, null);
